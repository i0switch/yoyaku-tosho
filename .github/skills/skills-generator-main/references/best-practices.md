...
- [ ] 日本語と英語の両方のキーワードがある
- [ ] 最大1024文字以内
- [ ] 必要なら「対象外」や「単純作業は除く」を入れて誤発火を防いでいる

## 1.5 先にユースケースを定義する

スキルを書き始める前に、最低2〜3件の具体的ユースケースを決める。
...
これを先に決めないと、`description` も本文も抽象化しすぎて弱くなる。
...
公式ガイドでは、スキルの良し悪しを感覚だけでなく、最低限の基準で見ていくのを推奨している。
...
必須ではないけど、以下のケースでは `metadata` を入れた方が運用しやすい。
...
## 8. 配布を見据えるなら README まで作る

公開・共有用のスキルは、SKILL.md だけだと不親切になりやすい。
