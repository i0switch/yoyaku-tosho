...
6. **具体的ユースケース**: どんな依頼で発火し、何を完了できれば成功か。最低2〜3件の具体例を言語化する
7. **成功基準**: 発火精度、完了までの流れ、失敗してはいけない条件を簡潔に決める

> 💡 ユーザーが「こんなことしたい」「あんな事ができたら」と漠然とした要望の場合は、目的から逆算して最適な構成を提案する。必要に応じて Generate + scripts/ の組み合わせを自動的に選択する。
>
> 💡 公式ガイド準拠で、**先にユースケースと成功基準を決めてから書き始める**。ここが曖昧なまま SKILL.md に入ると、発火条件も手順もぶれやすい。
...
5. **保守メタデータ**: 公開・共有や継続改善を見込むなら `metadata.version` `metadata.updated` `metadata.changelog` の追加を検討する
...
- [ ] `description` に対象外や競合しやすいケースへの境界が含まれているか
- [ ] スキル読み込み通知セクションが含まれているか
- [ ] SKILL.md が肥大化していないか（外部ファイル分離が適切か）
- [ ] 外部ファイルへのリンクが正しいか
- [ ] `scripts/` がある場合、スクリプトが安全で冪等か
- [ ] 2〜3件のユースケースと成功基準が定義されているか
- [ ] 直球発火 / 言い換え発火 / 非発火 の3系統でテストできる状態か
- [ ] 公開・共有するなら README・ライセンス・配布手順まで揃っているか
- [ ] 日本語で記述されているか

#### Step 7: テスト設計

スキル完成前に、最低でも以下の3種類のテストプロンプトを用意する。

1. **直球発火**: 誰が見てもこのスキルを使う依頼
2. **言い換え発火**: ユーザーが自然に言いそうな言い換え依頼
3. **非発火**: キーワードは近いが、本来このスキルを使うべきではない依頼
...
チーム共有や公開を見込むスキルでは、生成時点で以下も提案する。

- `README.md` の有無
- ライセンス表記
- セットアップ手順
- 依存関係や前提ツールの明記
- `.zip` 配布やフォルダ丸ごと共有を前提にした構成確認
- 変更履歴の更新方針
...
| テスト導線 | 直球発火・言い換え発火・非発火のテスト観点があるか |
| 運用保守 | metadata、README、ライセンス、配布手順が必要十分か |
...
- ユースケースや成功基準が書かれていない → 先に2〜3件の具体例と完了条件を追加
- 運用情報がない → `metadata`、README、配布手順を補強

#### Step 4: 改善結果の検証

- [ ] 改善後の `description` で意図通りに発火するか
- [ ] 直球発火 / 言い換え発火 / 非発火の3系統で挙動を確認したか
- [ ] 既存の機能が損なわれていないか
- [ ] 外部ファイルへのリンクが正しいか
- [ ] スクリプトが追加された場合、安全に動作するか
- [ ] 公開・共有を想定するなら README・ライセンス・変更履歴が揃っているか
...
| [references/testing-distribution-checklist.md](references/testing-distribution-checklist.md) | 発火テスト・配布・保守運用のチェックリスト |
