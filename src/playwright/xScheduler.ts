...
export async function scheduleXPost(
  ...
  try {
    ...
    if (!page.url().includes('x.com')) {
      logger.log(`Navigating to X home...`);
      await page.goto('https://x.com/home', { waitUntil: 'domcontentloaded', timeout: 60000 });
    } else {
      logger.log(`Already on X, clearing state...`);
    ...
    try {
      logger.log(`Attempting to click "Post" button...`);
      await page.waitForSelector(X_SELECTORS.postButton, { timeout: 5000 });
      await page.click(X_SELECTORS.postButton, { timeout: 5000 });
    } catch (e) {
      logger.log(`Sidebar button click failed or timed out. Navigating directly to /compose/post...`);
    ...
    logger.log(`Typing post content...`);
    ...
    logger.log(`Clicking schedule button...`);
    try {
      // Primary: data-testid
      await page.click(X_SELECTORS.scheduleButton, { timeout: 5000 });
    } catch (e) {
      logger.log(`Schedule button click failed, trying force click...`);
    ...
    const setSelectByLabel = async (labelText: string, value: string, indexFallback: number, fallbackLabels: string[] = []) => {
      ...
      try {
        ...
        if (await targetSelect.count() === 0) {
          logger.log(`Warning: Label "${labelText}" not found, using index fallback ${indexFallback}`);
    ...
    if (await confirmBtn.count() > 0) {
      ...
      // Emergency backup
      logger.log(`Warning: "${X_SELECTORS.buttonConfirm}" not found by text, trying data-testid...`);
    ...
    if (await finalBtn.count() > 0) {
      ...
      logger.log(`Warning: Final button with text "${X_SELECTORS.buttonSchedule}" not found, trying generic submit button...`);
    ...
    logger.error(`Failed to schedule post ${post.id}`, err);
